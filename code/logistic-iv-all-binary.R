...
if (tcond) {
  oracle <- plogis(sum(oparm * c(1, 1, 1))) * 0.5 + plogis(sum(oparm * c(1, 1, 0))) * 0.5 -
    plogis(sum(oparm * c(1, 0, 1))) * 0.5 - plogis(sum(oparm * c(1, 0, 0))) * 0.5
} else {
  oracle <- plogis(sum(oparm * c(1, 1, 0))) - plogis(sum(oparm * c(1, 0, 0)))
}

# FUNs --------------------------------------------------------------------

gen_dat <- function(n = tn, parmD = 1, parmY = oparm[1:2], discreteD = TRUE,
                    discreteE = TRUE, normalHDE = FALSE, conditional = tcond,
                    discreteH = TRUE, parmH = oparm[3]) {
...
res <- replicate(nsim, {
  ...
  d <- gen_dat(discreteD = TRUE, discreteE = TRUE, conditional = FALSE, discreteH = TRUE)
  ...
  YD <- diff(predict(mD, newdata = nd, type = "response"))

  ### Oracle conditional model
  ndH <- data.frame(expand.grid(D = c(0, 1), H = c(-1, 1)))
